...
try:
    from utils import (
        UNPOOLED_DATA_DIR, CLUSTERING_DIR, PUBLICATIONS, STRAIN_WISE_PUBLICATIONS,
        load_data, read_json_lists,
        identify_candidates, get_sequence_quicker, get_ohes, get_remaining_sequence, get_standard_feature, get_k_identities, get_kmer_jaccard_similiarities,
        transform_meta_features, _consensus_motif, _extract_junction_window, _get_motif_for_id, _hamming_distance
    )
except ImportError:
    import glob
...
def load_intersecting_clusters(strain, cutoff=0, clustering=None, logger=logging):
    ...
    if os.path.exists(intersect_path):
        ...
        if "Cluster" not in intersect_df.columns:
            ...
            if potentials:
                logger.debug(f'Found potential cluster columns: {potentials}')
...
def hdbscan_labels(embedding: np.ndarray, min_cluster_size: Optional[int] = None, min_samples: Optional[int] = None, param_grid: Optional[dict] = None, logger=logging):
    logger.info(f"Starting HDBSCAN clustering on embedding {embedding.shape} type {type(embedding)} with min_cluster_size={min_cluster_size}, min_samples={min_samples}, param_grid={param_grid}")
    if param_grid is not None:
        logger.debug(f"Starting HDBSCAN grid search with param_grid: {param_grid}")
        best_model = None
        best_score = -2.0
            ...
            for size in param_grid.get("min_cluster_size", [5] if min_cluster_size is None else [min_cluster_size]):
                for samples in param_grid.get("min_samples", [min_samples]):
                    try:
                        ...
                        logger.error(f"Error occurred while fitting HDBSCAN model with parameters (min_cluster_size: {size}, min_samples: {samples}, cluster_selection_epsilon: {epsilon}): {e}")
                        continue
                    ...
                    if score > best_score:
                        logger.debug(f"HDBSCAN grid search current best - min_cluster_size: {size}, min_samples: {samples}, cluster_selection_epsilon: {epsilon}, with #clusters: {unique_non_noise.size}, silhouette_score: {score} against previous {best_score}")
                        best_score = score
                        best_model = model
        ...
        logger.debug(f"Best HDBSCAN model - min_cluster_size: {best_model.min_cluster_size}, min_samples: {best_model.min_samples}, cluster_selection_epsilon: {best_model.cluster_selection_epsilon}, #clusters: {len(np.unique(best_model.labels_[best_model.labels_ >= 0]))}, silhouette_score: {best_score}")
    ...
    if min_cluster_size is not None:
        ...
        logger.debug(f"HDBSCAN - min_cluster_size: {model.min_cluster_size}, min_samples: {model.min_samples}, cluster_selection_epsilon: {model.cluster_selection_epsilon}, #clusters: {len(np.unique(model.labels_[model.labels_ >= 0]))}, silhouette_score: {silhouette_score(embedding[model.labels_ >= 0], model.labels_[model.labels_ >= 0]) if np.unique(model.labels_[model.labels_ >= 0]).size >= 2 else -1.0}")
    ...
    if max_size < 5:
        ...
        logger.debug(f"Not enough points for grid search, using min_cluster_size={model.min_cluster_size}. Silhouette score: {silhouette_score(embedding[model.labels_ >= 0], model.labels_[model.labels_ >= 0]) if np.unique(model.labels_[model.labels_ >= 0]).size >= 2 else -1.0}")
        return model.labels_, model
    best_model = None
    best_score = -2.0
...
def kmeans_labels(embedding: np.ndarray, k: Optional[int] = None, param_grid: Optional[dict] = None, logger=logging):
    if param_grid is not None and "k" in param_grid:
        logger.debug(f"Starting KMeans grid search with param_grid: {param_grid}")
        best_model = None
        best_score = -2.0
        for cur_k in param_grid.get("k", [5]):
            ...
            if score > best_score:
                logger.debug(f"KMeans grid search current best - k: {cur_k}, silhouette_score: {score} against previous {best_score}")
                best_score = score
                best_model = model
        ...
        if best_model is None:
            ...
            logger.debug(f"Not enough points for grid search, using k={best_model.n_clusters}. Silhouette score: {score}")
        logger.debug(f"Best KMeans model - k: {best_model.n_clusters}, silhouette_score: {best_score}")
        return best_model.labels_, best_model

    if k is not None:
        model = KMeans(n_clusters=k, max_iter=1000, random_state=42, copy_x=True).fit(embedding)
        logger.debug(f"KMeans - k: {model.n_clusters}, silhouette_score: {silhouette_score(embedding, model.labels_)}")
        return model.labels_, model

    max_k = min(20, len(embedding) - 1)
    if max_k < 2:
        model = KMeans(n_clusters=1, max_iter=1000, random_state=42, copy_x=True).fit(embedding)
        logger.debug(f"Not enough points for grid search, using k={model.n_clusters}. Silhouette score: {silhouette_score(embedding, model.labels_)}")
        return model.labels_, model
...
def find_closest_dvg(dataframe, strain=None, seg=None, start=None, end=None, id=None):
    '''
    Find the closest DVG on the same strain and segment to the given coordinates within the dataframe. Returns the ID of the closest DVG and the distance to it.
    '''
    if id is not None:
...
def cluster_intersecting_on_embedding(embedding_df, intersecting_ids, algorithm, logger=logging, **kwargs):
    intersecting_dataframe = embedding_df[embedding_df["ID"].isin(set(intersecting_ids))].copy()
    ...
    if algorithm == "hdbscan":
        ...
        labels, model = hdbscan_labels(intersecting_dataframe[['UMAP1', 'UMAP2']], param_grid=param_grid, logger=logger)
    elif algorithm == "kmeans":
        param_grid = kwargs.get("param_grid", DEFAULT_KMEANS_GRID)
        labels, model = kmeans_labels(intersecting_dataframe[['UMAP1', 'UMAP2']], param_grid=param_grid, logger=logger)
    else:
    ...
    logger.debug(f'Intersecting dataframe after reclustering: {intersecting_dataframe.shape[0]} rows and {intersecting_dataframe.shape[1]} columns. Intersecting Cluster column added with {intersecting_dataframe["Intersecting Cluster"].nunique()} unique clusters.\n{intersecting_dataframe.columns.tolist()}\n{intersecting_dataframe.head()}')
    ...
    logger.debug(f'Full dataframe after reclustering: {full_dataframe.shape[0]} rows and {full_dataframe.shape[1]} columns. Intersecting cluster column added with {full_dataframe["Intersecting Cluster"].nunique()} unique clusters.\n{full_dataframe.columns.tolist()}\n{full_dataframe.head()}')
...
def get_centroid_distance(dataframe, clustering_df, cluster_id_col="Cluster", centroid_cols=["centroid_x", "centroid_y"], coord_cols=["UMAP1", "UMAP2"], inplace=False, logger=logging, **kwargs):
    ...
    if missing_coords_mask.any():
        ...
        for id in missing_ids:
            ...
            if closest_id is not None:
                ...
                logger.warning(f"Could not find a DVG for ID {id}. Leaving coordinates as NaN.")
    if centroid_cols[0] not in clustering_df.columns or centroid_cols[1] not in clustering_df.columns:
        ...
        if "center_x" in clustering_df.columns and "center_y" in clustering_df.columns:
            ...
            logger.error(f"Could not find centroid columns in clustering dataframe. Expected one of: {centroid_cols}, ['center_x', 'center_y'], ['centroid_x', 'centroid_y']. Found columns: {clustering_df.columns.tolist()}")
            return df
...
def get_cluster_motif_identities(dataframe, clustering_data, flank=5, cluster_id_col="Cluster", centroid_cols=["centroid_x", "centroid_y"], coord_cols=["UMAP1", "UMAP2"], inplace=False, logger=logging, **kwargs):
    '''
    Adds motif-similarity by comparing each row's junction window to the consensus
    motif of each cluster via hamming-distance.
    '''
    clust_name = kwargs.get("cluster_name", None)
    if inplace:
        df = dataframe
...
def get_ngs_cluster_features(dataframe, ngs_clustering, inplace=False, name_prefix="", logger=logging, **kwargs):
    '''
    Add columns for each cluster in ngs_clustering, based on the NGS_read_count of each DVG in that cluster. The new columns will contain the normalized mean and CLR-encoded abundance of each cluster for each ACC_num group in the dataframe.
    '''
    if inplace:
        df = dataframe
    else:
    ...
    for cluster_id, group in ngs_clustering.groupby("Cluster"):
        ...
        df[f'{cluster_name}_mean_normed'] = 0.0
        #df[f'{cluster_name}_proportion'] = 0
        df[f'{cluster_name}_CLR'] = 0.0
    for acc_num, acc_group in df.groupby("ACC_num"):
        for cluster_id, clust_group in ngs_clustering.groupby("Cluster"):
            ...
            df.loc[acc_group.index, f'{cluster_name}_mean_normed'] = acc_group[acc_group["ID"].isin(cluster_dvgs)]["NGS_read_count"].mean() / acc_group["NGS_read_count"].max() if acc_group["NGS_read_count"].max() != 0 and pd.notna(acc_group[acc_group["ID"].isin(cluster_dvgs)]["NGS_read_count"].mean()) else 0.0
    ...
    for cluster_id in ngs_clustering["Cluster"].unique():
        if cluster_id == -1 or cluster_id == "-1":
            continue
        cluster_name = f"{name_prefix}{cluster_id}"
        for col in [f"{cluster_name}_CLR", f"{cluster_name}_mean_normed"]:
            if df[col].isna().any():
...
def load_preprocessed_data(strain, temp_id=0, logger=logging):
    if temp_id == 0:
    ...
    if not os.path.exists(data_path):
        logger.warning(f"Preprocessed data not found for strain {strain}. Expected at: {data_path}")
        return None, None
    if not os.path.exists(col_dict_path):
        logger.warning(f"Preprocessed column dictionary not found for strain {strain}. Expected at: {col_dict_path}")
...
def preprocess_strain_data(strain, logger=logging, **kwargs):
    ...
    if precomputed > 0 or kwargs.get("load_precomputed", False):
        logger.debug(f"Preprocessing for strain {strain} with precomputed: {precomputed} and load_precomputed: {kwargs.get('load_precomputed', False)}. Attempting to load preprocessed data...")
        data, col_dict = load_preprocessed_data(strain, temp_id=precomputed, logger=logger)
        if data is None:
        ...
        data = load_data(STRAIN_WISE_PUBLICATIONS.get(strain, PUBLICATIONS), unpooled=True)
    ...
    if kwargs.get("skip_dvg_identification", False) or precomputed >= 1:
        ...
        pub_ids = {pub_id: pub for pub_id, pub in enumerate(STRAIN_WISE_PUBLICATIONS[strain])}
    ...
    if True:  # kwargs.get("skip_vip_features", False) or precomputed >= 4:
    ...
    if kwargs.get("skip_clustering_features", False) or precomputed >= 5:
        ...
        for algorithm in ["hdbscan", "kmeans"]:
            for cutoff in cutoff_grid:
                for umap_name, load_func in [("scaff", load_scaffold), ("comb", load_comb_umap)]:
                    ...
                    if clustering_df is not None:
                        if kwargs.get("recluster",False):
                            ...
                            logger.info(f"Found {len(intersecting_ids)} intersecting candidates for reclustering.")
                            clustering_df = cluster_intersecting_on_embedding(clustering_df, intersecting_ids, algorithm, logger=logger, **kwargs.get("clustering_kwargs", {}))
                            ...
                            logger.info(f"Finished reclustering {algorithm} on {umap_name} embedding with cutoff {cutoff}. Clustering dataframe has {clustering_df.shape[0]} rows and {clustering_df.shape[1]} columns.")
                        else:
    ...
    if kwargs.get("skip_context_features", False) or precomputed >= 6:
        ...
        logger.debug(f"Calculating context features with starting shape {data.shape}")
        tmp_cols = set(data.columns.tolist())
        ...
        logger.debug(f"Finished calculating context features. New shape is {data.shape}.")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Preprocessing strain data.')
    ...
    logger.debug(f"Preprocessing completed for strain {strain}. Data has {data.shape[0]} rows and {data.shape[1]} columns. Column dictionary:\n{col_dict}\n{data.head()}")
